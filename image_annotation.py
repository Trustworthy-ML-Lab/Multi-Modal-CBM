...
def main():
    ...
    # load classes
    cls_file = data_utils.LABEL_FILES[dataset_name.split('_')[0]]
    ...
    if args.per_class_concept_file is not None:
        ...
        per_class_concepts_file = f"data/concept_sets/{dataset_name.split('_')[0]}_filtered_byclass.json"
    ...
    dataset = data_utils.get_data(dataset_name, preprocess=lambda x: transform(x, None)[0])
    ...
    for class_idx, class_name in enumerate(classes):
        ...
        prompt = format_concept(class_name) + " . "
        for concept in per_class_concept:
            prompt = prompt + f"{format_concept(concept)} . "
        prompt = prompt.strip()
        print(f"Prompt for class {class_name}: {prompt}")

        # only load images with class_idx
        dataset_subset = torch.utils.data.Subset(dataset, np.where(np.array(data_utils.get_targets_only(dataset_name)) == class_idx)[0])
